...
from izi_invoices.utils import InvoiceCreator
...
IZI_INVOICES_FOLDER_FORMATTED = 'invoices/{0}/{1:02d}/'.format(date.today().year, date.today().month)
FULL_PATH_TO_INVOICES = os.path.join(app_settings.IZI_INVOICES_DOCUMENTS_ROOT, IZI_INVOICES_FOLDER_FORMATTED)
...
class TestInvoice(WebTestCase):
    ...
    def tearDown(self):
        super().tearDown()
        # Remove `IZI_INVOICE_FOLDER` after each test
    ...
    @patch('izi_invoices.app_settings.IZI_INVOICES_INVOICE_MODEL', 'custom_invoices.CustomInvoice')
