...
* Merchant account, ``izi_invoices.models.LegalEntity``.
  In contains shop name, seller's business name, website, email, VAT number etc.

* Merchant address, ``izi_invoices.models.LegalEntityAddress``. It's
...
storage class ``izi_invoices.storages.DocumentsStorage``, invoice documents
placed into the nested folder ``settings.IZI_INVOICES_UPLOAD_FOLDER`` and
available for the admin users via dashboard order list.

In order to start generating invoices you need to integrate
``izi_invoices.utils.InvoiceCreator`` into your checkout flow, for instance
